...
# remove first row of data since it's for entirety of USA
education <- education[-1, ]
education <- education[, c("Federal Information Processing Standard (FIPS) Code", "Less than a high school diploma, 2017-21","High school diploma only, 2017-21", "Bachelor's degree or higher, 2017-21", "Percent of adults with less than a high school diploma, 2017-21","Percent of adults with a high school diploma only, 2017-21", "Percent of adults with a bachelor's degree or higher, 2017-21")]
...
fips_no_ed_data <- covid_yearly_final_ed %>%
  filter(is.na(high_school_diploma_rate) | is.na(bachelors_or_higher_rate) | is.na(No_degree)) %>%
...
### Is there a differece between those who had a Bacehlor's Degree in 2022 vs those who had less than a HS diploma?

```{r}
education_cases_2022 <- covid_yearly_final_ed %>%
  filter(year == 2022)

# Create the plot
ggplot(education_cases_2022) +
  ...
  labs(
    title = "Annual COVID-19 Cases per Capita vs. Education Rates for 2022",
    x = "Education Rate (%)",
    y = "Annual Cases per Capita"
  ) +
