...
head(all_region_mhhinc_weekly_localcor)
```
...
for (d in 1:length(datasets)) {
  ...
  for (p in 1:length(predictor)) {
      ...
      model_name <- paste(df_names[d],"_", predictor_names[p], sep = "")
...
```{r}
units = c(round(sd(weekly_spatial_metrics_waves$cor_lengths, na.rm = TRUE),2), round( sd(weekly_spatial_metrics_waves$r_0_50) ,2))
...
lm_model = simple_lm_models[[1]]

# 1. Linearity Check (Visual Inspection)
plot(lm_model)  # Plot diagnostics including residual vs. fitted values
...
vif(lm(log(next_week_marginal_cases) ~  cor_lengths + cor_lengths*same_tier + cor_lengths*tier_diff_1 + cor_lengths*tier_diff_2  , data = all_region_mhhinc_weekly_spatial_metrics))
vif(lm_scaled)

# Scaling the variables in the model
scaled_data <- within(all_region_mhhinc_weekly_spatial_metrics, {
  cor_lengths_scaled <- scale(cor_lengths)
  northeast_scaled <- scale(northeast)
  midwest_scaled <- scale(midwest)
  south_scaled <- scale(south)
  same_tier_scaled <- scale(same_tier)
  tier_diff_1_scaled <- scale(tier_diff_1)
  tier_diff_2_scaled <- scale(tier_diff_2)
})

# Fit the scaled model
lm_scaled <- lm(log(next_week_marginal_cases) ~ cor_lengths_scaled * northeast_scaled * same_tier_scaled + 
    cor_lengths_scaled * northeast_scaled * tier_diff_1_scaled + cor_lengths_scaled * northeast_scaled * 
    tier_diff_2_scaled + cor_lengths_scaled * midwest_scaled * same_tier_scaled + 
    cor_lengths_scaled * midwest_scaled * tier_diff_1_scaled + cor_lengths_scaled * midwest_scaled * 
    tier_diff_2_scaled + cor_lengths_scaled * south_scaled * same_tier_scaled + 
    cor_lengths_scaled * south_scaled * tier_diff_1_scaled + cor_lengths_scaled * south_scaled * 
    tier_diff_2_scaled, data = scaled_data)
summary(lm_model)
...
for (p in 1:length(predictor)) {
  for (c in 1:length(covariates)) {
    ...
    lm_model <- lm(lm_formula, data = all_region_mhhinc_weekly_localcor)
...
Seems like the only interactive term with significant results (for both models) is the same tier category in the South
...
print_coefficients <- function(coef_val, region, tier_diff, tier_descr) {
  if (!is.na(coef_val)) {
    coef <- coef_val + coefficients["r_0_50"]
    cat("- Among counties in the", region, "where the median household income tiers", tier_descr, "\n")
    cat("a 1 sd increase in local correlation (.19) corresponds to a ", 
        round(coef * .19* 100, 2), "% increase in cases in the following week\n", sep = "")}}
...
for (r in regions) {
  for (m in tier_diff) {
    coef_val <- coefficients[paste("r_0_50:factor(region)", r, ":", m, sep = "")]
    print_coefficients(coef_val, r, m, tier_descr[match(m, tier_diff)])}}

# Midwest Coefficients
for (m in tier_diff) {
  coef_val <- coefficients[paste("r_0_50:", m, sep = "")]
  print_coefficients(coef_val, "Midwest", m, tier_descr[match(m, tier_diff)])}

# Tier diff 3 counties
for (r in regions) {
  coef_val <- coefficients[paste("r_0_50:factor(region)", r, sep = "")]
  print_coefficients(coef_val, r, "tier_diff_3", "differ by 3")}

# Midwest & Tier diff 3
coef_val <- coefficients["r_0_50"]
print_coefficients(coef_val, "Midwest", "tier_diff_3", "differ by 3")
summary(lm_model)
```

Results we can report: Among counties in the **South** where the median
household income **tiers are the same** a 50 km increase in correlation
length corresponds to a *126.96%* change in cases in the following week
Among counties in the **Midwest** where the median household income
**tiers differ by 1** a 50 km increase in correlation length corresponds
to a *118.23%* change in cases in the following week

# Lauren: General + Wave Models (2 data sets, 8 models, 4 tables, 2 models/table)
...
*note from eleanor: what about in the west?*
...
*note from eleanor: let's drive our narrative around differing by tier 1 influencing cases the most (wave 2 in particular) in all counties*
