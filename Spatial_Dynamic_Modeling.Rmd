...
for (d in 1:length(datasets)) {
  ...
  for (p in 1:length(predictor)) {
    ...
    # For example:
    # pdf(file = paste(model_name_train, "_residual_plot.pdf", sep = ""))
    # plot(test_df$log(next_week_marginal_cases), residuals,
    #      main = paste("Residual Plot for", model_name_train),
    #      xlab = "Actual Values",
    #      ylab = "Residuals")
    # dev.off()
...
coefficients <- coef(region_lm_models_train[[5]])
...
coefficients <- coef(region_lm_models_train[[2]])
...
```{r}
# print results for significant interaction terms for urban counties
coefficients <- coef(region_lm_models_train[[6]])
...
```{r}
# print results for significant interaction terms for all counties
coefficients <- coef(region_lm_models_train[[3]])
...
```{r}
# print results for significant interaction terms for urban counties
coefficients <- coef(region_lm_models_train[[7]])
...
Regional effects are strong in all regions. Significant results across all waves, particularly strong regional effects in wave 1. Strange results in the Northeast in wave 1. Effects are not as discernible among Urban counties. Correlation length is better for predicting covid when discerning regional effects.
...
# Eleanor: Mask + Wave Models (1 data set, 4 models, 2 tables, 2 models/table)
...
# 5. No Perfect Multicollinearity Check
# Variance Inflation Factors (VIF)
vif(lm(log(next_week_marginal_cases) ~  cor_lengths + factor(wave) + factor(mhhinc_tier) , data = all_mask_weekly_spatial_metrics))
# Conditions not passed:
## Heteroskedasticity in 
## Non-linearity in 

summary(mask_lm_models_train[[1]])
```
