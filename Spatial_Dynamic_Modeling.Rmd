...
for (d in 1:length(datasets)) {
  ...
  for (p in 1:length(predictor)) {
    ...
    lm_formula_alt <- as.formula(paste("log(next_week_marginal_cases) ~", paste(predictor[p],"*wave_1 +"), paste(predictor[p],"*wave_2")))
    
    # Fit the linear model
    lm_model <- lm(lm_formula, data = df)
    lm_model_alt <- lm(lm_formula_alt, data = df)
    ...
    wave_lm_models_alt[[model_name]] <- lm_model_alt

  }
}
names(wave_lm_models_alt)
```
...
for (model in 1:length(wave_lm_models_alt)) {
  
  # Get model stats
  lm_model = wave_lm_models[[model]]
  coefficients <- coef(lm_model)
  coef_names <- names(coef(lm_model))
...
summary(wave_lm_models[[1]])
```

```{r}
summary(lm(log(next_week_marginal_cases) ~ cor_lengths*factor(wave), all_weekly_spatial_metrics_waves))
```
