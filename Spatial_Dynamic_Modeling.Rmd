...
# Create function that prints interpretation of coefficeints
print_interpretation <- function(interaction, region, tier_diff, tier_descr, significant) {
  if (!is.na(interaction) & significant == 1) {
    coef <- interaction + coefficients["r_0_50"]
    ...
    cat("a 1 sd increase in local correlation (.19) corresponds to a ", 
        round(coef * .19* 100, 2), "% increase in cases in the following week. Significant? ",significant == 1,"\n", sep = "")}}

# Create function that returns coefficient
print_coefficient <- function(interaction, region, tier_diff, tier_descr) {
...
is_significant <- function(coef, se, threshold = 1.96) {
...
for (r in 1:length(regions)) {
  ...
  for (m in 1:length(tier_diff)) {
    ...
    full_coef[i] = print_coefficient(interaction, region, tierdiff, tier_descr[m])
    
    # Print interpretation and significance
    significant[i] <- is_significant(as.numeric(full_coef[i]), as.numeric((se_total)))
    print_interpretation(interaction, region, tierdiff, tier_descr[m], significant[i])
    i <- i + 1
  }
}
```
