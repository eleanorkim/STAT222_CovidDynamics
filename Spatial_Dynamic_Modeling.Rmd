...
# Filtered data to same tiers
df_sametier = filter(all_region_mhhinc_weekly_spatial_metrics, same_tier == 1)
```
...
# Same MHHINC tier models
for (p in 1:length(predictor)) {
    lm_formula <- as.formula(paste("log(next_week_marginal_cases) ~ factor(region)*factor(mhhinc_tiers)*", predictor[p], collapse = " + "))
    lm_model <- lm(lm_formula, data = df_sametier)
   # Name model and save to list
    model_name <- paste("sametier_", pred_names[p], "_region", sep = "")
    lm_models[[model_name]] <- lm_model}
...
Seems like we only have significant interaction terms in the first model
...
cat("a 10 % increase in local correlation corresponds to a ", 
        round(coef * 10, 2), "% change in cases in the following week\n\n", sep = "")
```
