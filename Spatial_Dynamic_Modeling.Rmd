...
print_interpretation <- function(coef, wave= "every wave",region= "whole US", tier_descr, significant=TRUE, pred = predictor[2], county="all counties") {
  if (!is.na(coef) && isTRUE(significant)) {
    if (pred == predictor[1]) {
      factor_value = 254.5
      pred_type = "correlation length (254.5 km)"
    } else {
      factor_value = 0.19
      pred_type = "local correlation (0.19)"
    }
    cat("- For", county,"across", wave, "in the", region, "where the median household income tiers", tier_descr, "\n")
...
for (r in 1:length(regions)) {
  ...
  for (m in 1:length(tier_diff)) {
    ...
    print_interpretation(full_coef[i], wave= "every wave",region, tier_descr[m], significant[i], pred = "local_cor", county="all counties")
    i <- i + 1
  }
}
```

All results in the South & Midwest are significant. Higher effects among counties similar in mhhinc tier.

Only significant result form Urban county df: Midwest tiers differ by 1, 15.24% increase

# Lauren: Wave Models (2 data sets, 4 models, 2 tables, 2 models/table)
*hey lauren! the simple/general models are done, so it's just the wave dummy models for cor_length / local_cor and urban / all counties -e*
...
## Heteroskedastic in all_corlengths_wave_region, all_corlengths_region, all_localcor_region,all_localcor_wave_region,  urban_corlengths_region, urban_localcor_region
## Non-linearity in all_localcor_wave_region, all_localcor_region, urban_corlengths_region, urban_localcor_wave_region, urban_localcor_region
## solutions --> robust standard errors? don't trust low significance
```
...
for (model in 1:length(region_lm_models)) {
  ...
  if (model %% 2 == 0) {
    ...
    for (r in 1:length(regions)) {
      ...
      print_interpretation(full_coef1[j], wave, region, "are disregarded", significant1[j], pred, county)}}
  ...
  for (w in 1:length(waves)) {
    ...
    # Print interpretation and significance
    significant[i] <- is_significant(as.numeric(full_coef[i]), as.numeric((se_total)))
    print_interpretation(full_coef[i], wave, region, "are disregarded", significant[i], pred, county)
    i <- i + 1
    }}}
}
```

Urban counties in the West in wave 1 and Northeast counties in wave 1 experienced extremeley strong spatial spread. Regional effects differed across the time period, but especially so in wave 1.  
...
# Conditions not passed:
## Heteroskedasticity in all_localcor_mhhinc, urban_localcor_wave_mhhinc, urban_localcor_mhhinc
## Non-linearity in all_localcor_wave_mhhinc, all_localcor_mhhinc, urban_localcor_wave_mhhinc, urban_localcor_mhhinc
## These are the local correlation models with mhhinc
```
...
for (model in 1:length(mhhinc_lm_models)) {
  ...
  if (model %% 2 == 0) {
    ...
    for (m in 1:length(tier_diff)) {
      ...
      # Print interpretation and significance
      significant1[j] <- is_significant(as.numeric(full_coef1[j]), as.numeric((se_total)))
      print_interpretation(full_coef1[j], wave, region="whole US", tier_descr[m], significant1[j], pred, county)}}
  ...
  for (w in 1:length(waves)) {
    ...
    # Print interpretation and significance
    significant[i] <- is_significant(as.numeric(full_coef[i]), as.numeric((se_total)))
    print_interpretation(full_coef[i], wave, region="whole US", tier_descr[m], significant[i], pred, county)
    i <- i + 1
    }}}
}
```
...
Significant differential effects among "equality level" - across every wave but especially in wave 3. Spread is greater among counties of similar socioeconomic level (considering all counties).
