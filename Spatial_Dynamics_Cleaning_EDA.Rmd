...
```{r}
# R always terminates when I do a for loop so i just run this chunk 4 times for each region
# table(county_week$region)
select_region = "West"
...
week_start = 154
week_end = 165
subset_county_week = county_week
subset_pairwise_counties = pairwise_counties
...
for (w in week_start:week_end) {
    ...
    # Save to vector/list
    cor_lengths_W[w-week_start+1] =  xi
    list_Cr_W[[w-week_start+1]] <- C_r_T
}
```
...
all_list_Cr <- vector("list", length = 16)
all_cor_lengths <- vector("list", length = 16)
...
for (select_tier1 in 1:4) {
  ...
  for (select_tier2 in 1:4) {
    ...
    all_list_Cr[[counter]] <- list_Cr
    all_cor_lengths[[counter]] <- cor_lengths
...
# Now you have 16 lists containing your results
# You can access them using all_list_Cr and all_cor_lengths
...
# saveRDS(c(cor_lengths, cor_lengths_W), "all_correlation_lengths.rds")
# saveRDS(c(list_Cr, list_Cr_W), "all_Cr.rds")
...
# cut out weeks 6-8
list_Cr = list_Cr[4:160]
cor_lengths = cor_lengths[4:160]
...
# cut out weeks 6-8
Cr_regions_less <- lapply(Cr_regions, function(x) x[4:160])
cor_lengths_regions_less <- lapply(cor_lengths_regions, function(x) x[4:160])
...
for (r in 1:4) {
  list_Cr = Cr_regions_less[[r]]
  cor_lengths = cor_lengths_regions_less[[r]]
...
# save to cs
# write.csv(regional_weekly_spatial_metrics, "regional_weekly_spatial_metrics.csv")
```
...
# cut out weeks 6-8
Cr_poverty_less <- lapply(Cr_poverty, function(x) x[4:160])
cor_lengths_poverty_less <- lapply(cor_lengths_poverty, function(x) x[4:160])
...
# Weekly Spatial Metrics for Regions
for (r in 1:6) {
  list_Cr = Cr_poverty_less[[r]]
  cor_lengths = cor_lengths_poverty_less[[r]]
...
# save to cs
# write.csv(poverty_weekly_spatial_metrics, "poverty_weekly_spatial_metrics.csv")
```
...
# cut out weeks 6-8
Cr_mhhinc_less <- lapply(Cr_mhhinc, function(x) x[4:160])
cor_lengths_mhhinc_less <- lapply(cor_lengths_mhhinc, function(x) x[4:160])

# initialize data frame
mhhinc_weekly_spatial_metrics <- data.frame(matrix(NA, nrow = 0, ncol = 54))
colnames(mhhinc_weekly_spatial_metrics)[1:4] <- c("week_start_date", "total_cases", "marginal_cases", "cor_lengths")
colnames(mhhinc_weekly_spatial_metrics)[5:52] <- as.character(col_names)
colnames(mhhinc_weekly_spatial_metrics)[53:54] <- c("next_week_marginal_cases","mhhinc_tier")

# Weekly Spatial Metrics for Regions
for (r in 1:16) {
  list_Cr = Cr_mhhinc_less[[r]]
  cor_lengths = cor_lengths_mhhinc_less[[r]]
  ...
  weekly_spatial_metrics <- weekly_spatial_metrics %>%
    mutate(next_week_marginal_cases = dplyr::lead(marginal_cases))
  weekly_spatial_metrics$next_week_marginal_cases[(n-1):n] = c(NA, NA)
  weekly_spatial_metrics$mhhinc_tier = rep(r, n)
  mhhinc_weekly_spatial_metrics = rbind(mhhinc_weekly_spatial_metrics, weekly_spatial_metrics)
}

# Add wave column
mhhinc_weekly_spatial_metrics <- mhhinc_weekly_spatial_metrics %>%
  mutate(
    wave = case_when(
      as.Date(week_start_date) >= "2021-12-11" ~ 3,
      as.Date(week_start_date) >= "2020-12-26" ~ 2,
...
dummy_col_names <- paste0("tier_", rep(1:4, each = 4), "_", rep(1:4, times = 4))
...
#cut out unnecessary dummies
columns_to_remove <- c("tier_2_1", "tier_3_2", "tier_3_1", "tier_4_1", "tier_4_3", "tier_4_2")
mhhinc_weekly_spatial_metrics <- mhhinc_weekly_spatial_metrics[, !names(mhhinc_weekly_spatial_metrics) %in% columns_to_remove]
values_to_remove <- c(5, 9, 10, 13, 14, 15)
mhhinc_weekly_spatial_metrics <- mhhinc_weekly_spatial_metrics[!mhhinc_weekly_spatial_metrics$mhhinc_tier %in% values_to_remove, ]

# Group dummies by tier difference
mhhinc_weekly_spatial_metrics$same_tier <- ifelse(mhhinc_weekly_spatial_metrics$tier_1_1 == 1 | mhhinc_weekly_spatial_metrics$tier_2_2 == 1 | mhhinc_weekly_spatial_metrics$tier_3_3 == 1 | mhhinc_weekly_spatial_metrics$tier_4_4 == 1, 1, 0)
mhhinc_weekly_spatial_metrics$tier_diff_1 <- ifelse(mhhinc_weekly_spatial_metrics$tier_1_2 == 1 | mhhinc_weekly_spatial_metrics$tier_2_3 == 1 | mhhinc_weekly_spatial_metrics$tier_3_4 == 1, 1, 0)
mhhinc_weekly_spatial_metrics$tier_diff_2 <- ifelse(mhhinc_weekly_spatial_metrics$tier_1_3 == 1 | mhhinc_weekly_spatial_metrics$tier_2_4 == 1, 1, 0)
# save to cs
# write.csv(mhhinc_weekly_spatial_metrics, "mhhinc_weekly_spatial_metrics.csv")
```
