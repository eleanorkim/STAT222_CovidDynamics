...
poverty = read_xlsx("Poverty_Tiers.xlsx")
...
covid_daily_avg_pop <- covid_daily_avg %>%
    ...
    select(fips, county, region, week, total_cases, total_deaths, 
...
# R always terminates when I do a for loop so i just run this chunk 4 times for each region
select_tier = 3
subset_pairwise_counties = filter(pairwise_counties, Tier_Poverty_1 == select_tier & Tier_Poverty_2 ==select_tier)
subset_county_week = filter(county_week, Tier_Poverty == select_tier)
...
for (w in week_start:week_end) {
    ...
    # Save to vector/list
    cor_lengths_3[w-week_start+1] =  xi
    list_Cr_3[[w-week_start+1]] <- C_r_T
}
```
...
saveRDS(cor_lengths_3, "tier3_correlation_lengths.rds")
saveRDS(list_Cr_3, "tier3_Cr.rds")
...
# read in the region subsetted spatial metrics from weeks 6-165
list_Cr_NE <- readRDS("NE_Cr.rds")
cor_lengths_NE <- readRDS("NE_correlation_lengths.rds")
list_Cr_MW <- readRDS("MW_Cr.rds")
cor_lengths_MW <- readRDS("MW_correlation_lengths.rds")
list_Cr_S <- readRDS("S_Cr.rds")
cor_lengths_S <- readRDS("S_correlation_lengths.rds")
list_Cr_W <- readRDS("W_Cr.rds")
cor_lengths_W <- readRDS("W_correlation_lengths.rds")
# combine
Cr_regions = list(list_Cr_NE,list_Cr_MW, list_Cr_S, list_Cr_W)
cor_lengths_regions = list(cor_lengths_NE, cor_lengths_MW, cor_lengths_S, cor_lengths_W)

# read in poverty subsetted spatial metrics from weeks 6-165
list_Cr_1 <- readRDS("tier1_Cr.rds")
cor_lengths_1 <- readRDS("tier1_correlation_lengths.rds")
list_Cr_2 <- readRDS("tier2_Cr.rds")
cor_lengths_2 <- readRDS("tier2_correlation_lengths.rds")
list_Cr_3 <- readRDS("tier3_Cr.rds")
cor_lengths_3 <- readRDS("tier3_correlation_lengths.rds")
# combine
Cr_poverty = list(list_Cr_1,list_Cr_2, list_Cr_3)
cor_lengths_poverty = list(cor_lengths_1, cor_lengths_2, cor_lengths_3)
```
...
weekly_spatial_metrics$next_week_marginal_cases[159:160] = c(NA, NA)
...
for (r in 1:4) {
  list_Cr = Cr_regions[[r]]
  cor_lengths = cor_lengths_regions[[r]]
  ...
  weekly_spatial_metrics <- weekly_spatial_metrics %>%
    mutate(next_week_marginal_cases = dplyr::lead(marginal_cases))
  weekly_spatial_metrics$next_week_marginal_cases[159:160] = c(NA, NA)
  weekly_spatial_metrics$region = rep(regions[r], 160)
...
regional_weekly_spatial_metrics$northeast[weekly_spatial_metrics$region != "Northeast"] = 0
regional_weekly_spatial_metrics$midwest[regional_weekly_spatial_metrics$region == "Midwest"] = 1
regional_weekly_spatial_metrics$midwest[regional_weekly_spatial_metrics$region != "Midwest"] = 0
regional_weekly_spatial_metrics$south[regional_weekly_spatial_metrics$region == "South"] = 1
regional_weekly_spatial_metrics$south[regional_weekly_spatial_metrics$region != "South"] = 0
...
# Weekly Spatial Metrics for Regions
for (r in 1:3) {
  list_Cr = Cr_poverty[[r]]
  cor_lengths = cor_lengths_poverty[[r]]
  ...
  weekly_spatial_metrics <- weekly_spatial_metrics %>%
    mutate(next_week_marginal_cases = dplyr::lead(marginal_cases))
  weekly_spatial_metrics$next_week_marginal_cases[159:160] = c(NA, NA)
  weekly_spatial_metrics$poverty_tier = rep(r, 160)
  poverty_weekly_spatial_metrics = rbind(poverty_weekly_spatial_metrics, weekly_spatial_metrics)
}

# add wave dummies
poverty_weekly_spatial_metrics$tier_1[poverty_weekly_spatial_metrics$poverty_tier == 1] = 1
poverty_weekly_spatial_metrics$tier_1[poverty_weekly_spatial_metrics$poverty_tier != 1] = 0
poverty_weekly_spatial_metrics$tier_2[poverty_weekly_spatial_metrics$poverty_tier == 2] = 1
poverty_weekly_spatial_metrics$tier_2[poverty_weekly_spatial_metrics$poverty_tier != 2] = 0
...
saveWorkbook(wb, "Poverty_Tiers.xlsx", overwrite = TRUE)
```
