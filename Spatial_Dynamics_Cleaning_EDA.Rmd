...
# Inspect
nrow(county_df) #256 urban counties, 2155 total
head(county_df)
```
...
```{r}
# add mask usage tiers to county_df
mask = read.csv("mask-use-by-county.csv")
mask$COUNTYFP <- sprintf("%05d", mask$COUNTYFP)
summary(mask$ALWAYS)
mask <- mask %>%
  mutate(mask_usage = case_when(
    ALWAYS < 0.4 ~ 1,
    ALWAYS >= 0.4 & ALWAYS < 0.5 ~ 2,
    ALWAYS >= 0.5 & ALWAYS < 0.6 ~ 3,
    ALWAYS >= 0.6 ~ 4
  ))
table(mask$mask_usage)
county_df <- merge(county_df, mask, by.x = "fips", by.y = "COUNTYFP", all.x = TRUE)
head(county_df)
```
...
```
### stat 135 final question prep

```{r}
#  data and model
nrow(county_week)
head(county_week)
summary(lm(total_cases ~ total_deaths + PCTPOVALL_2021, county_week))
covid = filter(county_week, county == "San Francisco")
covid$cases = covid$total_cases
covid$deaths = covid$total_deaths+2000
covid <- covid %>%
  mutate(cases_last = lag(total_cases))
covid <- covid %>%
  mutate(deaths_last = lag(total_deaths))

model <- lm(deaths ~ cases , data = covid)
summary(model)
model2 <- lm(deaths ~ cases + cases_last, data = covid)
summary(model2)
cor(covid$cases, covid$cases_last, use = 'complete.obs')

# Summary of the model
model_summary$coefficients[, "Estimate"] <- signif(model_summary$coefficients[, "Estimate"], digits = 2)
model_summary$coefficients[, "Std. Error"] <- signif(model_summary$coefficients[, "Std. Error"], digits = 2)
...
# Residual plot
plot(x = model$fitted.values, y = residuals(model),
     xlab = "Fitted values", ylab = "Residuals",
     main = "Residual Plot",
     pch = 19)
abline(h = 0, col = "red")
mean(covid_sf$weekly_cases)
covid = covid_sf
.9*2000/.006 = 300,000
View(covid)
...
# Assuming 'model' is your lm object
...
# Set up the plotting area to have 1 row and 2 columns, and decrease axis font size
par(mfrow = c(1, 2), cex.axis = 0.7) # Adjust the value of cex.axis as needed

# Normal QQ plot
qqnorm(residuals(model), main = "Normal Q-Q Plot", pch = 19, cex = .3)
...
model1 <- lm(deaths ~ cases , data = covid)
model2 <- lm(cases ~ deaths , data = covid)
summary(model1)
summary(model2)
```
...
# add mask tiers to county_week
county_week<- merge(county_week, mask, by.x = "fips", by.y = "COUNTYFP", all.x = TRUE)
...
# Plotting code
plot(r_lower, C_r_T, type = "l", xlab = "Distance Between County Centroids r (km)", ylab = "Spatial Correlation in Covid Cases C(r)", main = paste("Spatial Correlation and Correlation Length in Week", T), col="grey", cex.lab=.8, lwd=3)
# Add vertical lines
abline(v = 370, col = "red", lty = 3) 
abline(v = 50, col = "blue", lty = 3) 
abline(h = 0, col = "black", lty = 1) 
# Add intersection points
points(370, C_r_T[18], col="red", pch=19)  # pch=19 makes a solid circle
points(50, 0.42, col="blue", pch=19)

# Adding legend
legend("topright", legend = c("C(r)", "local correlation", "correlation length"), col = c("grey", "blue", "red"), lty = c(1, 3, 3), pch = c(NA, 19, 19))
```
...
# Initialize lists to store results
region_mhhinc_list_localcor <- vector("list", length = 40)
# up until week 5 we have < 22 observations
week_start <- 9
week_end <- 165

local_pairwise_counties = filter(pairwise_counties, r_i ==1)
# Initialize counter for indexing
counter <- 1
regions <- c("Northeast", "Midwest", "South", "West")
for (select_region in regions) {
  for (select_tier1 in 1:4) {
    ...
    for (select_tier2 in select_tier1:4) {
      # Subset pairwise_counties and county_week based on select_tier1 and select_tier2
      subset_pairwise_counties <- filter(local_pairwise_counties, region_1 == select_region & Tier_MedHHInc_1 == select_tier1 & Tier_MedHHInc_2 == select_tier2)
      subset_county_week <- filter(county_week, region == select_region & Tier_MedHHInc == select_tier1 | Tier_MedHHInc == select_tier2)  
      
      # Initialize vectors
      localcor <- numeric(length = week_end - week_start + 1)
      ...
      for (w in week_start:week_end) {
        ...
        # Remove rows where X_T is NA (no covid data for county pair)
        covid_counties_1_2 <- covid_counties_1_2[complete.cases(covid_counties_1_2), ]
        
        # Calculate local correlation for obs ≥ 4
        if (nrow(covid_counties_1_2) >= 4) {
          X_T_1_r <- covid_counties_1_2$X_T_1
          X_T_2_r <- covid_counties_1_2$X_T_2
          c <- cor(X_T_1_r, X_T_2_r, use = "complete.obs")
        } else {
          c <- NA
        }
        
        localcor[w - week_start + 1] <- c
      }
      
      # Save results to list
      region_mhhinc_list_localcor[[counter]] <- localcor
...
```{r}
# Initialize lists to store results
mask_list_Cr <- vector("list", length = 10)
mask_cor_lengths <- vector("list", length = 10)
...
for (select_tier1 in 1:4) {
  ...
  for (select_tier2 in select_tier1:4) {
    # Subset pairwise_counties and county_week based on select_tier1 and select_tier2
    subset_pairwise_counties <- filter(pairwise_counties, mask_usage_1 == select_tier1 & mask_usage_2 == select_tier2)
    subset_county_week <- filter(county_week, mask_usage == select_tier1 | mask_usage == select_tier2)
    ...
    # Save results to list
   mask_list_Cr[[counter]] <- list_Cr
    mask_cor_lengths[[counter]] <- cor_lengths
    
    # Increment counter
    counter <- counter + 1
  }
}
length(mask_list_Cr[[1]])
length(mask_cor_lengths)
```
...
saveRDS(mask_list_Cr, "mask_list_Cr.rds")
saveRDS(mask_cor_lengths, "mask_correlation_lengths.rds")
...
localcor_region_mhhinc <- readRDS("region_mhhinc_list_localcor.rds")
urban_localcor_region_mhhinc <- readRDS("urban_region_mhhinc_list_localcor.rds")

# mask
Cr_mask <- readRDS("mask_list_Cr.rds")
cor_lengths_mask <- readRDS("mask_correlation_lengths.rds")
```
...
# prepare total new cases column
total_deaths_by_week <- county_week %>%
  group_by(week) %>%
  summarize(total_deaths = sum(total_deaths, na.rm = TRUE))
total_deaths = total_deaths_by_week[8:164,2]
```

### Whole Data (Waves) Weekly Spatial Metrics
```{r}
# Weekly Spatial Metrics for Entire data set
cor_lengths = cor_lengths[-c(1:3)]
list_Cr = list_Cr[-c(1:3)]
n = length(cor_lengths)
# Create dataframe
...
weekly_spatial_metrics$total_deaths = unlist(total_deaths)
weekly_spatial_metrics[157,57]
weekly_spatial_metrics$week = c(6:162)

summary(weekly_spatial_metrics[,c("week_start_date","next_week_marginal_cases","cor_lengths","r_0_50")])
names(weekly_spatial_metrics)
# save to cs
# write.csv(weekly_spatial_metrics, "weekly_spatial_metrics_waves.csv")
```

### Regional Weekly Spatial Metrics

```{r}
# For Regions
n = length(urban_Cr_regions[[1]])
...
### Poverty Weekly Spatial Metrics
...
### MHHINC/Regional Weekly Spatial Metrics
...
region_mhhinc_weekly_spatial_metrics$tier_diff_3 <- ifelse(region_mhhinc_weekly_spatial_metrics$mhhinc_tiers == "1_4",
       1, 0)
# Define the variable based on tier difference
region_mhhinc_weekly_spatial_metrics <- region_mhhinc_weekly_spatial_metrics %>%
  mutate(tier_descr = case_when(
    same_tier == 1 ~ "are the same",
    tier_diff_1 == 1 ~ "differ by 1",
    tier_diff_2 == 1 ~ "differ by 2",
    tier_diff_3 == 1 ~ "differ by 3",
...
### MHHINC + Regional Local Correlation Only

```{r}
# initialize data frame
region_mhhinc_weekly_localcor <- data.frame(matrix(NA, nrow = 0, ncol = 4))
colnames(region_mhhinc_weekly_localcor) <- c("week_start_date", "total_cases", "marginal_cases", "r_0_50")

# Weekly Spatial Metrics for MHHINC
for (r in 1:40) {
  localcor = localcor_region_mhhinc[[r]]
  
  # Create dataframe
  weekly_spatial_metrics <- data.frame(matrix(NA, nrow = length(localcor), ncol = 4))
  colnames(weekly_spatial_metrics) <- c("week_start_date", "total_cases", "marginal_cases", "r_0_50")
  
  # Fill in first few columns
  weekly_spatial_metrics[, "week_start_date"] <- date_names
  weekly_spatial_metrics[, "total_cases"] <- total_cases
  weekly_spatial_metrics[, "marginal_cases"] <- c(0, diff(weekly_spatial_metrics$total_cases))
  weekly_spatial_metrics[, "r_0_50"] <- localcor
  
  # add next week marginal cases column
  weekly_spatial_metrics <- weekly_spatial_metrics %>%
    mutate(next_week_marginal_cases = dplyr::lead(marginal_cases))
  weekly_spatial_metrics$next_week_marginal_cases[(n-1):n] = c(NA, NA)
  weekly_spatial_metrics$block = rep(r, 157)
  region_mhhinc_weekly_localcor = rbind(region_mhhinc_weekly_localcor, weekly_spatial_metrics)
}

# Add wave column
region_mhhinc_weekly_localcor <- region_mhhinc_weekly_localcor %>%
  mutate(
    wave = case_when(
      as.Date(week_start_date) >= "2021-12-11" ~ 3,
      as.Date(week_start_date) >= "2020-12-26" ~ 2,
...
for (block_value in 1:40) {
  region_mhhinc_weekly_localcor$region[region_mhhinc_weekly_localcor$block == block_value] = combo[block_value,1]
  region_mhhinc_weekly_localcor$mhhinc_tiers[region_mhhinc_weekly_localcor$block == block_value] = combo[block_value,2]
}
```
...
# Initialize the dummy variable columns with all zeros
region_mhhinc_weekly_localcor[, dummy_col_names] <- 0
...
for (select_tier1 in 1:4) {
  for (select_tier2 in select_tier1:4) {
    ...
    # Update the dummy variable column based on the conditions
    region_mhhinc_weekly_localcor[[dummy_col_name]] <- ifelse(region_mhhinc_weekly_localcor$mhhinc_tier == ((select_tier1 - 1) * 4 + select_tier2), 1, 0)
  }
}
```
...
```{r}
# Group dummies by tier difference
region_mhhinc_weekly_localcor$same_tier <- ifelse(region_mhhinc_weekly_localcor$mhhinc_tiers == "1_1" | 
       region_mhhinc_weekly_localcor$mhhinc_tiers == "2_2" |
       region_mhhinc_weekly_localcor$mhhinc_tiers == "3_3" |
       region_mhhinc_weekly_localcor$mhhinc_tiers == "4_4",
       1, 0)
region_mhhinc_weekly_localcor$tier_diff_1 <- ifelse(region_mhhinc_weekly_localcor$mhhinc_tiers == "1_2"|
       region_mhhinc_weekly_localcor$mhhinc_tiers == "2_3" |
       region_mhhinc_weekly_localcor$mhhinc_tiers == "3_4",
       1, 0)
region_mhhinc_weekly_localcor$tier_diff_2 <- ifelse(region_mhhinc_weekly_localcor$mhhinc_tiers == "1_3"|
       region_mhhinc_weekly_localcor$mhhinc_tiers == "2_4",
       1, 0)
region_mhhinc_weekly_localcor$tier_diff_3 <- ifelse(region_mhhinc_weekly_localcor$mhhinc_tiers == "1_4",
       1, 0)
...
# save to cs
# write.csv(region_mhhinc_weekly_localcor, "region_mhhinc_weekly_localcor.csv")
```

```{r}
fix = read.csv("mhhinc_weekly_spatial_metrics.csv")
names(fix)
fix$tier_diff_3 <- ifelse(fix$mhhinc_tier == "1_4",
       1, 0)
# Define the variable based on tier difference
fix <- fix %>%
  mutate(tier_descr = case_when(
...
# write.csv(fix, "mhhinc_weekly_spatial_metrics.csv")

```


### Masks Weekly Spatial Metrics

```{r}
# For Mask "Always" Usage
n = length(Cr_mask[[1]])

# initialize data frame
mask_weekly_spatial_metrics <- data.frame(matrix(NA, nrow = 0, ncol = 54))
colnames(mask_weekly_spatial_metrics)[1:4] <- c("week_start_date", "total_cases", "marginal_cases", "cor_lengths")
colnames(mask_weekly_spatial_metrics)[5:52] <- as.character(col_names)
colnames(mask_weekly_spatial_metrics)[53:54] <- c("next_week_marginal_cases","mask_usage")

# Weekly Spatial Metrics for Regions
for (r in 1:10) {
  list_Cr = Cr_mask[[r]]
  cor_lengths = cor_lengths_mask[[r]]
  mask_usages = c("1_1","1_2","1_3","1_4","2_2","2_3","2_4","3_3","3_4","4_4")
  ...
  weekly_spatial_metrics <- weekly_spatial_metrics %>%
    mutate(next_week_marginal_cases = dplyr::lead(marginal_cases))
  weekly_spatial_metrics$next_week_marginal_cases[(n-1):n] = c(NA, NA)
  weekly_spatial_metrics$mask_usage = rep(mask_usages[r], n)
  mask_weekly_spatial_metrics = rbind(mask_weekly_spatial_metrics, weekly_spatial_metrics)
}
```


```{r}
# add mask tier dummies
mask_weekly_spatial_metrics$mask_11 <- ifelse(mask_weekly_spatial_metrics$mask_usage == "1_1", 1, 0)
mask_weekly_spatial_metrics$mask_12 <- ifelse(mask_weekly_spatial_metrics$mask_usage == "1_2", 1, 0)
mask_weekly_spatial_metrics$mask_13 <- ifelse(mask_weekly_spatial_metrics$mask_usage == "1_3", 1, 0)
mask_weekly_spatial_metrics$mask_14 <- ifelse(mask_weekly_spatial_metrics$mask_usage == "1_4", 1, 0)
mask_weekly_spatial_metrics$mask_22 <- ifelse(mask_weekly_spatial_metrics$mask_usage == "2_2", 1, 0)
mask_weekly_spatial_metrics$mask_23 <- ifelse(mask_weekly_spatial_metrics$mask_usage == "2_3", 1, 0)
mask_weekly_spatial_metrics$mask_24 <- ifelse(mask_weekly_spatial_metrics$mask_usage == "2_4", 1, 0)
mask_weekly_spatial_metrics$mask_33 <- ifelse(mask_weekly_spatial_metrics$mask_usage == "3_3", 1, 0)
mask_weekly_spatial_metrics$mask_34 <- ifelse(mask_weekly_spatial_metrics$mask_usage == "3_4", 1, 0)
mask_weekly_spatial_metrics$mask_44 <- ifelse(mask_weekly_spatial_metrics$mask_usage == "4_4", 1, 0)

# Add wave column
mask_weekly_spatial_metrics <- mask_weekly_spatial_metrics %>%
  mutate(
    wave = case_when(
      as.Date(week_start_date) >= "2021-12-11" ~ 3,
      as.Date(week_start_date) >= "2020-12-26" ~ 2,
...
# Add mask tier group dummies
mask_weekly_spatial_metrics$same_usage <- ifelse(mask_weekly_spatial_metrics$mask_usage == "1_1" | 
       mask_weekly_spatial_metrics$mask_usage == "2_2" |
       mask_weekly_spatial_metrics$mask_usage == "3_3" |
       mask_weekly_spatial_metrics$mask_usage == "4_4",
       1, 0)
mask_weekly_spatial_metrics$usage_diff_1 <- ifelse(mask_weekly_spatial_metrics$mask_usage == "1_2"|
       mask_weekly_spatial_metrics$mask_usage == "2_3" |
       mask_weekly_spatial_metrics$mask_usage == "3_4",
       1, 0)
mask_weekly_spatial_metrics$usage_diff_2 <- ifelse(mask_weekly_spatial_metrics$mask_usage == "1_3"|
       mask_weekly_spatial_metrics$mask_usage == "2_4",
       1, 0)
mask_weekly_spatial_metrics$usage_diff_3 <- ifelse(mask_weekly_spatial_metrics$mask_usage == "1_4",
       1, 0)
# Define the variable based on tier difference
mask_weekly_spatial_metrics <- mask_weekly_spatial_metrics %>%
  mutate(mask_use_descr = case_when(
    same_usage == 1 ~ "are the same",
    usage_diff_1 == 1 ~ "differ by 1",
    usage_diff_2 == 1 ~ "differ by 2",
    usage_diff_3 == 1 ~ "differ by 3",
    TRUE ~ NA_character_  # Handle other cases (if any)
  ))

all_mask_weekly_spatial_metrics = mask_weekly_spatial_metrics
# save to cs
write.csv(all_mask_weekly_spatial_metrics, "all_mask_weekly_spatial_metrics.csv")
```
...
df_long <- gather(all_weekly_spatial_metrics_waves, key = "r", value = "value", -week_start_date, -total_cases, -marginal_cases, -cor_lengths, -next_week_marginal_cases)
...
cor(weekly_spatial_metrics$r_0_50, weekly_spatial_metrics$cor_lengths, use = "complete.obs")

# Scatter Plot
plot(weekly_spatial_metrics$cor_lengths, weekly_spatial_metrics$r_0_50,  pch = 16, ylab = "Local Correlation C(r<50)", xlab = "Correlation Length xi")
```
...
for (i in 1:4) { # looking at effects in the 4 following weeks 
  print(cor(log(weekly_spatial_metrics$cor_lengths[-seq_len(i)]), log(weekly_spatial_metrics$marginal_cases[-seq_len(i)]), use = "complete.obs"))
} # decreases from .44 correlation

cor(log(weekly_spatial_metrics$cor_lengths), log(weekly_spatial_metrics$next_week_marginal_cases), use = "complete.obs")

#Linear-Linear Scatter Plot
plot( weekly_spatial_metrics$cor_lengths, weekly_spatial_metrics$next_week_marginal_cases,pch = 16, ylab = "Marginal Case Increase", xlab = "Correlation Length (km)", main="Linear-Linear")

# Linear-Log Scatter Plot
plot(weekly_spatial_metrics$cor_lengths, log(weekly_spatial_metrics$next_week_marginal_cases),pch = 16, ylab = "Log(Marginal Case Increase)", xlab = "Correlation Length (km)", main="Log-Linear")

# Log-Log Scatter Plot
plot( log(weekly_spatial_metrics$cor_lengths), log(weekly_spatial_metrics$next_week_marginal_cases),pch = 16, ylab = "Log(Marginal Case Increase)", xlab = "Log(Correlation Length)", main="Log-Log")
```
...
```{r}
# TRUE DATA
N = length(log(weekly_spatial_metrics$cor_lengths))-1
p = 1
X = cbind(rep(1,N), weekly_spatial_metrics$cor_lengths[1:N])
...
#write to csv
write.csv(full_betas, "log_lin_model_coefs.csv")
```
...
weekly_summary1 = filter(weekly_summary1, week < 160)
