...
# Filter data
data = filter(merged_data, distance_km <= 1000)
...
# Add r col
data$r <- round(r_upper[data$r_i],2)

# Inspect distribution of r_i
data$r_i = as.numeric(data$r_i)
hist(data$r_i)
table(data$r_i)
```
...
merged_data <- merge(data, covid_100[, c("fips", "X_T")], by.x = "fips1", by.y = "fips", all.x = TRUE, suffixes = c("", "_1"))
names(merged_data)[names(merged_data) == "X_T"] <- "X_T_1"

# Merge data frames based on fips2
merged_data <- merge(merged_data, covid_100[, c("fips", "X_T")], by.x = "fips2", by.y = "fips", all.x = TRUE, suffixes = c("", "_2"))
names(merged_data)[names(merged_data) == "X_T"] <- "X_T_2"
...
for (i in 1:n) {
  subset_data_1 <- merged_data$X_T_1[merged_data$r_i == i]
  subset_data_2 <- merged_data$X_T_2[merged_data$r_i == i]
...
for (i in 1:n) {
  subset_data_1 <- merged_data$X_T_1[merged_data$r_i == i]
  subset_data_2 <- merged_data$X_T_2[merged_data$r_i == i]
  numerator = sum(subset_data_1*subset_data_2 - m_1[i]*m_2[i], na.rm = TRUE)/length(merged_data$r_i == i)
...
plot(r_lower, C_r, type = "l", xlab = "r_lower", ylab = "C_r", main = "C_r against r_lower")
...
for (w in 1:10) {
  ...
  # Merge data frames based on fips1
  merged_data <- merge(data, covid_week[, c("fips", "X_T")], by.x = "fips1", by.y = "fips", all.x = TRUE, suffixes = c("", "_1"))
  names(merged_data)[names(merged_data) == "X_T"] <- "X_T_1"

  # Merge data frames based on fips2
  merged_data <- merge(merged_data, covid_week[, c("fips", "X_T")], by.x = "fips2", by.y = "fips", all.x = TRUE, suffixes = c("", "_2"))
...
```
```{r}
# Convert list_Cr to a matrix
matrix_Cr <- sapply(list_Cr, unlist)
# Plot using matplot
matplot(r_lower, matrix_Cr, type = "l", col = 1:10, lty = 1:10,
        xlab = "r_lower", ylab = "C_r", main = "C_r against r_lower for 10 weeks",
        legend.text = seq_along(list_Cr), col.axis = "blue")
legend("topright", legend = seq_along(list_Cr), col = 1:10, lty = 1:10, cex = 0.8)
