...
# Creat X_T column <-- is this right?
...
# Correlation function
  ...
  C_r_T[i]= numerator/denominator # burh this is literally just cor(X_T_1, X_T_2)
...
for (w in 6:153) {
  ...
  # Correlation function
    ...
    # is C_r_T significant?
    significant[i] = (cor.test(X_T_1_r, X_T_2_r)$p.value < .000001)
...
cor_lengths_1 = unlist(cor_lengths_1[6:153])
cor_lengths_2 = unlist(cor_lengths_2[6:153])
...
legend_labels <- paste("Plot", seq_along(cor_lengths))  # Adjust labels as needed
for (i in seq_along(cor_lengths)) {
  abline(v = cor_lengths_2[i], lty = 2, col = i)
}
```
...
names(combined_data) = c("Distance (km)", date_names)

# Make a copy with number columns
combined_data_copy = combined_data
names(combined_data_copy) = c("Distance (km)", c(1:153))
...
weeks_aggregated = cbind(aggregated_data, weekly_data)
